  ...
  formObject(req).then((newObj) => {
    ...
    try {
      MongoClient.connect(db_conn_str, (err, db) => {
        if (err) {
          console.error(err);
          res.status(500).json( {message: 'Could not connect to database', error: err} );
        }
...
function mongoPut(req, res, next) {
  ...
  formObject(req).then((updateObj) => {
    ...
    try {
      MongoClient.connect(db_conn_str, (err, db) => {
        db.db('recipe-studio').collection('recipes')
        .updateOne( { _id: new mongodb.ObjectID(rid) }, {$set: updateObj}, (err, r) => {
          if (err) { 
            console.error(err);
...
function formObject(req) {
  ...
  if (req.method == 'POST') {
    ...
    return new Promise((resolve, reject) => {
      ...
      // set user
      let a = validateAuthor(req.body.author)
      .then((author) => {
        objFields.author = author;
      });

      // set ingredients
      let i = validateIngredients(req.body.ingredients)
      .then((ingredients) => {
